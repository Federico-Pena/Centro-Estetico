...
function Calendario() {
  const [fechaReserva, setFechaReserva] = useState(HOY_STRING_BIEN.split('T')[0])
  ...
  const actualizarReservaNueva = (reserva) => {
    if (reserva) {
      setOpenFormulario(false)
      const reservasGuardadas = reservasSemanales.filter((res) => res._id !== reserva._id)
      const reservadasGuardadas = reservadas.filter((res) => res._id !== reserva._id)
      setReservadas([...reservadasGuardadas, reserva])
  ...
  const reservarHora = (e) => {
    ...
    const comprobación = compararFechas(new Date(horaSeleccionada), reservasSemanales)
  ...
  const actualizarBorrada = (datos) => {
    const { reserva } = datos
    if (reserva) {
      const mensaje = `Reserva borrada de ${reserva.pacienteNombre} el dia ${formatFechaParaUser({
  ...
  return (
    <>
      ...
      <main className='containerCalendario'>
        <>
          <ContadorReservas reservas={reservasSemanales} mostrarReservadas={mostrarReservadas} />
          <h1>Hoy {formatFechaParaUser({ fecha: HOY_STRING_BIEN })}</h1>
          <div className='containerBotones'>
            <BotónSecundario onClickFunction={semanaAnterior} texto={'Anterior'} />
            {loading && <LoaderChico />}
            <BotónSecundario onClickFunction={semanaSiguiente} texto={'Siguiente'} />
          </div>
              ...
              return (
                !(dia.dia.getDay() === 0) && (
                  <div key={dia.dia} className={`dias`}>
                    <h3
                      className={`${
                        dia.dia.getDate() === HOY.getDate() && dia.dia.getDay() === HOY.getDay()
                          ? 'esHoy'
                          : ''
                      }`}>
                      {formatFechaParaUser({
                          ...
                          i == 0 || i == 1
                            ...
                            : horaReservada &&
                              ...
                              (horaReservada.estado === ESTADOS_RESERVAS.pago ||
                                horaReservada.estado === ESTADOS_RESERVAS.pendiente)
                            ? `paraReservaAdmin ${horaReservada.estado}`
                            : horaReservada
