      ...
      {openInfo ? (
        <div className='fixed inset-0 bg-color-logo grid place-content-center p-4 z-30'>
          <article
            ...
            <header className='bg-slate-50 grid grid-flow-col place-content-center gap-8 p-4 sticky top-0 w-full'>
              <button
                  ...
                  {servicio.tratamientos.map(
                    (sesión, i) =>
                      i === 0 && (
                        <ul
                          key={i}
                          className='flex-1 py-4 px-8 list-disc list-inside grid items-end'>
        ...
        <article
          className={`${isVisible ? 'animate-fadeIn' : ''} ${
            className ? className : ''
          } flex flex-col border border-black rounded-lg overflow-hidden gap-8 max-w-md`}
          key={servicio._id}
          ref={imgRef}>
          {isVisible ? (
