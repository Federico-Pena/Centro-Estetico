...
import { ESTADOS_RESERVAS, HOY_STRING_BIEN, MESES } from '../../constantes'
        ...
        <>
          <FormularioReservaAdmin
            reserva={{
              ...
              pacienteNombre: hora === '08:00' || hora === '08:30' ? 'admin' : null
            }}
            setForm={() => setOpenFormulario(false)}
            actualizarReserva={(res) => {
              actualizarReservas(res)
              res.reserva && setOpenFormulario(false)
            }}
          />
        </>
      )}
      ...
      <div className='select-container-container'>
        ...
        <select
          ...
          {fechas?.length > 0 &&
            fechas?.map((dia) => {
              return (
                dia !== HOY_STRING_BIEN.split('T')[0] && (
                  <option className='selectedComponent-option' key={dia} value={dia}>
