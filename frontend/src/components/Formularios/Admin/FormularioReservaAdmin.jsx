...
import { ESTADOS_RESERVAS, HOY_STRING_BIEN } from '../../../constantes'
...
export const FormularioReservaAdmin = ({ reserva, setForm, editar, actualizarReserva }) => {
  const [dia, setDia] = useState(
    new Date(`${reserva.fecha}`).toISOString().split('T')[0] || HOY_STRING_BIEN.split('T')[0]
  )
  const [horaForm, setHoraForm] = useState(formatHoraUser(new Date(`${reserva.fecha}`)) || '')
  const [reservas, setReservas] = useState([])
  const [motivo, setMotivo] = useState(reserva.tratamiento?.nombre || '')
  const [sesion, setSesion] = useState(reserva.tratamiento?.descripcionSesion || '')
  ...
  const { nombres, pagina, setPagina, totalPacientes, totalPaginas } = usePaciente()
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      await fetchData(`${apiEndPoint.reservas.agregar}`, options, (res) => {
        const { reserva, error, mensaje } = res
        if (error) {
          setMensaje(error)
        } else {
          setMensaje(mensaje)
  ...
  const handleSubmitEditar = async (e) => {
    ...
    if (
      ...
      await fetchData(url, options, (response) => {
        const { reserva, error, mensaje } = response
        if (error) {
          setMensaje(error)
        } else {
          setMensaje(mensaje)
      ...
      <form
        ...
        <div className='input'>
          ...
          <input
            ...
            name='fecha'
            required
            disabled={inputNombre === 'admin' || reserva.pacienteNombre ? false : true}
        ...
        <div className='input'>
          <label htmlFor='observaciones'>
            {inputNombre === 'admin' ? 'Tareas. Separadas por comas' : 'Observaciones'}
          </label>
          <input
        ...
        <div className='input'>
          ...
          {dia && (
            <span className='spanHora'>
              Dia <strong>{formatFechaParaUser({ fecha: new Date(`${dia} ${horaForm}`) })}</strong>
            </span>
          )}
          {horaForm && (
            <span className='spanHora'>
