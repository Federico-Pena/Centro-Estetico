  ...
  try {
    ...
    const pacienteUpdate = {
      ...
      contactoEmergencia: {
        ...
        telefonoContacto2: telefonoContacto2 || pacienteAnterior.telefonoContacto2
      },
    ...
    const paciente = await Paciente.findByIdAndUpdate(req.params.id, pacienteUpdate)
    if (!paciente) {
      return res.status(404).json({ error: 'El paciente no existe' })
    }
    const nuevoPaciente = await Paciente.findOne(paciente._id)
    const mensaje = `Paciente nuevo ${nuevoPaciente.nombre}.`
    res.status(200).json({ nuevoPaciente, mensaje })
  } catch (error) {
    console.log(error)
